...
class Recipe(models.Model):
    ...
    name = models.CharField(
        max_length=200, verbose_name="食谱名称", help_text="食谱的标题"
    )
    ...
    cooking_time = models.IntegerField(
        default=0, verbose_name="烹饪时间", help_text="制作该食谱所需的时间（分钟）"
    )

    # 份数
    servings = models.IntegerField(
        default=1, verbose_name="份数", help_text="该食谱可制作的份数"
    )
    ...
    description = models.TextField(
        blank=True, null=True, verbose_name="简介", help_text="食谱的详细描述"
    )

    # 浏览次数
    views = models.IntegerField(
        default=0, verbose_name="浏览次数", help_text="食谱被浏览的次数"
    )

    # 点赞数
    likes = models.IntegerField(
        default=0, verbose_name="点赞数", help_text="食谱被点赞的次数"
    )

    # 收藏数
    favorites = models.IntegerField(
        default=0, verbose_name="收藏数", help_text="食谱被收藏的次数"
    )

    # 是否发布
    is_published = models.BooleanField(
        default=True, verbose_name="是否发布", help_text="是否公开显示该食谱"
    )

    # 创建时间
    created_at = models.DateTimeField(
        auto_now_add=True, verbose_name="创建时间", help_text="食谱创建的时间"
    )

    # 更新时间
    updated_at = models.DateTimeField(
        auto_now=True, verbose_name="更新时间", help_text="食谱最后更新的时间"
    )
...
class RecipeIngredient(models.Model):
    ...
    quantity = models.DecimalField(
        max_digits=10, decimal_places=2, verbose_name="数量", help_text="食材的用量"
    )
    ...
    is_main = models.BooleanField(
        default=False, verbose_name="是否主料", help_text="是否为该食谱的主要食材"
    )
...
class CookingStep(models.Model):
    ...
    step_number = models.IntegerField(
        verbose_name="步骤序号", help_text="该步骤在食谱中的顺序（从 1 开始）"
    )

    # 步骤描述
    description = models.TextField(
        verbose_name="步骤描述", help_text="该步骤的详细说明"
    )
    ...
    duration = models.IntegerField(
        default=0, verbose_name="耗时", help_text="完成该步骤所需的时间（分钟）"
    )

    # 小贴士
    tips = models.TextField(
        blank=True, null=True, verbose_name="小贴士", help_text="该步骤的注意事项或技巧"
    )
...
class UserBehavior(models.Model):
    ...
    # 创建时间
    created_at = models.DateTimeField(
        auto_now_add=True, verbose_name="创建时间", help_text="行为发生的时间"
    )
