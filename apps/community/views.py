...
class FoodPostListView(APIView):
    ...
    def get(self, request):
        ...
        if page is not None:
            serializer = FoodPostSerializer(page, many=True, context={'request': request})
            result = paginator.get_paginated_response(serializer.data)
            return success_response(data=result.data, message='获取动态列表成功')

        serializer = FoodPostSerializer(queryset, many=True, context={'request': request})
...
class FoodPostLikeView(APIView):
    ...
    def post(self, request, post_id):
        ...
        if like:
            ...
            post.save()
            return success_response(message='取消点赞成功', data={'is_liked': False, 'likes': post.likes})
        else:
            PostLike.objects.create(user=request.user, post=post)
            post.likes += 1
            post.save()
            return success_response(message='点赞成功', data={'is_liked': True, 'likes': post.likes})
