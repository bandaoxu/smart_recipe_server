...
- **食谱管理**: 完整的 CRUD 操作、点赞 toggle、收藏 toggle、搜索、我的食谱、我的喜欢
- **食材管理**: 食材列表、搜索、营养信息、应季推荐、创建食谱时自动 get_or_create
- **购物清单**: 清单管理、基于食谱自动生成
- **社区功能**: 动态发布/列表/详情、评论、动态点赞 toggle（PostLike 模型）
- **权限控制**: 基于 JWT 的细粒度权限管理
- **统一响应**: 标准化的 API 响应格式
- **文件上传**: 图片上传接口（/api/upload/），支持 media 静态服务
...
- **代码行数**: 3000+ 行（含详细中文注释）
- **API 端点**: 50+ 个
- **数据表**: 11 个业务表（含 PostLike）+ 11 个 Django 内置表
