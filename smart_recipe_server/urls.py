...
def upload_file(request):
    ...
    if not file:
        return Response(
            {"code": 400, "message": "未提供文件", "data": None}, status=400
        )
...
def admin_upload_image(request):
    """Admin 专用图片上传视图（Session 认证，仅限 staff 用户）"""
    file = request.FILES.get("file")
    if not file:
    ...
    if ext not in allowed:
        return JsonResponse(
            {"error": "不支持的图片格式，请上传 jpg/png/gif/webp"}, status=400
        )
    filename = f"{uuid.uuid4().hex}{ext}"
    upload_dir = os.path.join(settings.MEDIA_ROOT, "uploads")
    os.makedirs(upload_dir, exist_ok=True)
...
urlpatterns = [
    ...
    # Admin 专用图片上传（Session 认证）
    path("admin/upload-image/", admin_upload_image, name="admin_upload_image"),
    # Django Admin 后台管理
